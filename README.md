...
启动后打开浏览器访问 `http://localhost:8080`，进入桌面式 Web 管理界面。页面包含开机过渡动画、顶部菜单栏、底部应用栏和可拖拽窗口。默认提供四个应用窗口：

- **终端窗口**：多标签命令行，支持彩色输出、滚动查看、快捷输入
- **文件窗口**：目录树浏览，支持展开/折叠、右键菜单、新建与删除
- **监控窗口**：文件系统、块组、进程、内存的实时状态展示
- **文本编辑窗口**：浏览器内直接打开、修改、保存虚拟磁盘中的文本文件

支持深浅主题切换、窗口最小化/最大化、右键桌面快捷打开应用。文本编辑保存后，终端和文件窗口可看到同步结果。
...
├── GET /         → HTTP 响应 → 桌面式单页应用（HTML/CSS/JS 内嵌于 C 源码）
