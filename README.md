...
启动后打开浏览器访问 `http://localhost:8080`，进入 Web 桌面界面。页面包含启动过渡动画、菜单栏、Dock 和可拖拽窗口，提供终端、文件浏览、监控和文本编辑四个窗口。

也可通过 `nc localhost 4096` 连接原始终端。

> **首次使用**：先在终端中执行 `format` 格式化磁盘，创建用户后再进行文件编辑、编译和运行测试。
...
src/
  ...
  web_page.h          # 内嵌的桌面式 Web UI 页面（开机过渡、窗口系统、Dock、主题切换）
  editor.h/c          # 内置文本编辑器
  fs/                 # 文件系统层
...
├── GET /         → HTTP 响应 → 单页 Web 界面（HTML/CSS/JS 内嵌于 C 源码）
├── /ws/N         → WebSocket → 终端会话子进程
└── /api          → WebSocket → API 会话子进程
```

`--serve` 模式下，服务进程作为父进程运行事件循环，对每个连接创建子进程：
- 终端连接对应一个交互式命令会话，行为与本地终端一致
- API 连接负责接收页面请求，执行文件浏览、文本读取、文本保存、监控查询等操作
...
| 区域 | 功能 | 数据来源 |
|------|------|----------|
| 桌面层 | 开机过渡动画、壁纸背景、菜单栏、应用栏、主题切换 | 页面本地逻辑 |
| 终端窗口 | 多标签命令行、命令输入、彩色输出渲染 | `/ws/N` WebSocket |
| 文件窗口 | 树形浏览、右键操作、双击打开文件 | `/api` WebSocket |
| 文本编辑窗口 | 打开文件、在线修改、保存写回虚拟磁盘 | `/api` WebSocket |
| 监控窗口 | 文件系统状态、块组统计、进程列表、内存热力图（定时刷新） | `/api` WebSocket |
...
| 保存文本 | `{"cmd":"write","path":"/f","data":"...文本内容..."}` | 保存完成返回 `ok:true` |
...
- 前端 HTML/CSS/JS 内嵌在 `web_page.h`，无外部依赖
- 终端输出自行解析 ANSI 颜色码
- API 请求使用手写 JSON 解析器（扁平对象）
- 文件浏览走 `/api`，不解析终端输出
